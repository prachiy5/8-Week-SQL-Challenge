...
select cast(
			(select number_of_churned_cust_after_trial from cte2 ) as float)*100/count(distinct customer_id) as churn_percentage_right_after_trial 
			from subscriptions
...
select count(distinct customer_id) as num_cutsomers from subscriptions s inner join plans p 
on s.plan_id=p.plan_id
where datename(year,start_date)=2020 and plan_name='pro annual'
...
with cte as (select customer_id, p.plan_id, plan_name, start_date,
lead(plan_name,1) over(partition by customer_id order by start_date) as next_plan from subscriptions s inner join plans p 
on p.plan_id=s.plan_id)

select count(*) as downgraded_customers from cte
